...
def make_hybrid_optimizer(total_steps: int):
    ...
    def label_fn(params):
        # FIX: optax.multi_transform calls `param_labels(params)` with the WHOLE params
        # pytree as a single argument and expects back a pytree of labels with the same
        # structure -- it does NOT call a per-leaf (path, param) function itself.
        # tree_map_with_path is what supplies (path, param) per leaf; label_fn just wraps it.
