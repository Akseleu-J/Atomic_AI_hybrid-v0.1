...
def burst_damper(decay: float = 0.98, threshold_ratio: float = 3.0, min_scale: float = 0.1):
    ...
    ФИКС (этот пасс -- включение в make_hybrid_optimizer): раньше эта
    функция существовала в файле, но НИКОГДА не вызывалась в
    make_hybrid_optimizer's tx.chain -- т.е. была написана, но фактически
    не подключена (мёртвый код). Диагноз по свежим данным (router_max_cos/
    min_col_norm полностью здоровы -- MoE не источник взрыва; сам burst
    случается на РАЗНЫХ % warmup при РАЗНОЙ длине total_steps, но на
    похожем АБСОЛЮТНОМ числе шагов честного непрерывного momentum --
    указывает на медленный дрейф момента/весов, а не на архитектурную
    нестабильность, завязанную на позицию LR-кривой) говорит, что нужна
    ИМЕННО контекстно-адаптивная защита, а не более жёсткий статичный
    clip -- burst_damper сейчас подключается первым элементом chain, см.
    make_hybrid_optimizer ниже."""
    def init_fn(params):
...
def make_hybrid_optimizer(total_steps: int, muon_diagnostic_disable: bool = False):
    ...
    # ФИКС (этот пасс -- ГЛАВНОЕ включение burst_damper): функция
    # существовала в файле, но НИКОГДА не вызывалась здесь -- см. её
    # собственный докстринг выше про диагноз (router здоров, MoE не
    # причина; дрейф происходит по числу пройденных шагов честного
    # momentum, не по позиции на warmup-кривой -- нужна контекстно-
    # адаптивная защита, а не более жёсткий статичный global-norm clip).
    # Вставляется ПЕРВЫМ элементом chain -- работает на СЫРОМ градиенте,
    # до clip_by_global_norm и до multi_transform-разметки по группам.
    # BurstDamperState.ema_norm -- новый персистентный скаляр в opt_state;
    # т.к. это НОВЫЙ NamedTuple-узел chain'а, при restore со старого
    # чекпоинта (сохранённого ДО этого патча) train.py's
    # _generic_pytree_merge корректно оставит его со свежей инициализацией
    # (ema_norm=1.0) -- ветка "несовпадение длины list/tuple" в
    # _generic_pytree_merge сработает на самом tx.chain-tuple (длина
    # выросла на 1 элемент), поэтому ПЕРВЫЙ resume после этого патча
    # оставит ВЕСЬ chain (не только damper) со свежей инициализацией --
    # см. предупреждение в конце этого сообщения.
    damper_tx = burst_damper(decay=0.98, threshold_ratio=3.0, min_scale=0.1)
