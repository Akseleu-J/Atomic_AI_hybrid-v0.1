...
(unchanged from the original project file -- already has the two-line
decay_diff clip + nan_to_num defenses added after the step-710 incident.)
"""
...
def _build_chunk_wy(q_c, k_c, v_c, g_raw_c, b_c, w_c, scale):
    ...
    # Line-of-defense #1 (pre-existing, step-710 incident): decay_diff should
    # be <=0 for causal pairs but numerically can leak positive -> exp(inf)
    # without this clip. Kept exactly as before.
    ...
    # Line-of-defense #2: this is the path that ACTUALLY runs in backward
    # (via jax.vjp in kernel_trainable.py), independent of the Pallas
    # forward's own sanitization.
